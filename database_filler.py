...
if __name__ == "__main__":
	...
	while True:
		...
		for bus_input_list in json_vehiclepositions["features"]:
			...
			result = sql_get_result(mycursor, 'SELECT COUNT(trip_id) FROM trips WHERE trip_id = %s LIMIT 1', (trip_id, ))[0][0]

			delay = int(bus_input_list["properties"]["last_position"]["delay"])
			shape_traveled = int(float(bus_input_list["properties"]["last_position"]["gtfs_shape_dist_traveled"]) * 1000)
			...
			if result == 0:
				...
				result = sql_run_transaction_and_fetch(mydb, mycursor, 'SELECT insert_headsign_if_exists_and_return_id(%s)', (trip_headsign, ))[0][0]
				sql_run_transaction(mydb, mycursor, 'INSERT INTO trips (trip_id, trip_headsign_id, current_delay, shape_traveled, trip_no) VALUES (%s, %s, %s, %s, %s)', (trip_id, result, delay, shape_traveled, trip_no))
