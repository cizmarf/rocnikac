...
class TestDatabase(unittest.TestCase):
	...
	def testInsertTestData(self):
		...
		trips = database_connection.execute_fetchall("""
			SELECT count(*) FROM trips""")
	...
	def test_get_all_pairs_of_stops(self):
		...
		all_trip_coordinates = \
			database_connection.execute_procedure_fetchall('get_all_pairs_of_stops', (0,0, 1500))

		ride_of_trip = database_connection.execute_fetchall("""
			SELECT * 
			FROM rides 
			WHERE id_trip = %s 
			ORDER BY shape_dist_traveled""",
		(all_trip_coordinates[0][0],))
